...
class Trainer_with_distillation(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        student_hidden_state = student_hidden_states.view(-1, student_hidden_states.shape[-1])
        teacher_hidden_state = teacher_hidden_states.view(-1, student_hidden_states.shape[-1])
        huber_loss = torch.nn.functional.huber_loss(input = student_hidden_state, target = teacher_hidden_state, reduction = 'mean')
...
class Distiller(BaseTuner):
    """
    Initializes the `Distiller` class with given arguments.
    ...
    def tune(self, model, ref_model, dataset, transform_dataset_in_place=True):
        """
        ...
        if model_args.use_lora:
            ...
            DistillingTrainer = Trainer_with_distillation
            trainer_callbacks = []

        trainer = DistillingTrainer(
