...
def get_stack(tokenizer, n_samples, seq_len):
    dataset_path = 'data/redpajama-4g/stack'
    block_size = seq_len
    data_args = DatasetArguments(dataset_path=dataset_path, block_size=block_size, max_train_samples=n_samples)
    dataset = Dataset(data_args)
    traindata = dataset.get_backend_dataset()
...
def get_github(tokenizer, n_samples, seq_len):
    dataset_path = 'data/redpajama-4g/github'
    block_size = seq_len
    data_args = DatasetArguments(dataset_path=dataset_path, block_size=block_size, max_train_samples=n_samples)
    dataset = Dataset(data_args)
    traindata = dataset.get_backend_dataset()
...
def get_examples(dataset, tokenizer, n_samples, seq_len = 128):
    if dataset == 'c4':
        ...
        return get_stack(tokenizer, n_samples, seq_len)
    elif dataset == 'github':
        return get_github(tokenizer, n_samples, seq_len)
    else:
