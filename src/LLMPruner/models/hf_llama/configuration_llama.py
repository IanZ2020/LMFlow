...
class PrunedLlamaConfig(PretrainedConfig):
    r"""
    This is the configuration class to store the configuration of a [`LlamaModel`]. It is used to instantiate an LLaMA
    model according to the specified arguments, defining the model architecture. Instantiating a configuration with the
    defaults will yield a similar configuration to that of the LLaMA-7B.
    ...
    def get_llama_config(self, id, **kwargs):
        return LlamaConfig(
            vocab_size=self.vocab_size,
            hidden_size=self.hidden_size,
            intermediate_size=self.intermediate_size[id],
            num_hidden_layers=self.num_hidden_layers,
            num_attention_heads=self.num_attention_heads[id],
            num_key_value_heads=self.num_key_value_heads[id],
    ...
    def from_llama_config(self, config: LlamaConfig):
        ...
        self.num_key_value_heads = [self.num_key_value_heads for _ in range(self.num_hidden_layers)]
